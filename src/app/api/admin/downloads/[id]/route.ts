...
async function parseDownloadForm(request: Request) {
  ...
  return { data: parsed.data, uploadedFile: Boolean(uploadedFile) };
}
...
export async function PATCH(request: Request, { params }: Params) {
  ...
  try {
    ...
    if (parsed.uploadedFile && existing.storageKey !== download.storageKey) {
      await deleteLocalStorageFile(existing.storageKey).catch(() => undefined);
    }
    ...
    return NextResponse.json({ ok: false, message: "Unable to update download." }, { status: 500 });
  }
}
...
export async function DELETE(_request: Request, { params }: Params) {
  ...
  const { id } = await params;
  const existing = await prisma.productDownload.findUnique({ where: { id }, select: { storageKey: true, filename: true } });
