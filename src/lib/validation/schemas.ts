...
export const licenseCreateSchema = z.object({
  ...
  licenseType: z.enum(["Permanent", "Monthly", "Yearly", "Lifetime", "Trial", "Custom"]).default("Lifetime"),
  minimumVersion: z.string().trim().max(80).optional().or(z.literal("")),
  allowedVersionsJson: z.string().max(4000).optional().or(z.literal("")),
